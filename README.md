...
- 搜索：歌曲、歌手、专辑、歌单
- 用户系统：注册、登录、退出、收藏歌曲、收藏歌单、最近播放（含时间戳）
...
├── styles/          全局样式（app.css）
├── utils/           工具（notice / openAuthWindow / pageCss / storage）
├── views/           页面组件
├── App.vue
└── main.js
```
...
- 首页等旧静态 CSS（`public/assets/css/`）仍在部分页面通过 `pageCss.js` 动态加载，已知存在切换时无样式闪现（FOUC）问题，待后续稳妥解决。