...
- **用户前台**：浏览曲库、搜索、播放、收藏、歌单、播放历史、个人中心和歌词展示。
- **管理后台**：管理员登录，歌曲、歌手、专辑、分类的增删改查，音频上传、编辑、上下架与批量操作。
...
- **歌词**：支持 LRC 歌词展示与逐字染色，兼容 UTF-8 / GBK / UTF-16 编码。
- **可视化**：歌词面板内嵌 Canvas 频谱音柱（山峰延绵动效），随音乐实时跳动。
...
│   ├── components/           通用组件（PlayerBar 全局播放器、PlayerPanel 歌词面板、AppHeader 顶栏、Icon 图标等）
│   ├── router/               路由配置
