...
- 搜索：热门搜索 / 历史搜索、歌曲 / 歌单 / 歌手三分类、关键词高亮、相关性排序、整行播放、`/` 快捷键聚焦
...
├── src/
│   ├── components/   通用组件（AppHeader / PlayerBar / UserCard / Icon）
...
- 首页、登录、注册的旧静态 CSS（`public/assets/css/`）仍通过 `pageCss.js` 动态加载，未纳入 Prettier 格式化。
