...
- 歌曲管理：本地上传（FFmpeg 转 MP3）、远程 URL 录入、弹窗编辑（可替换封面 / 歌词）、试听、上下架、删除与批量操作。
- 歌手、专辑、分类管理：分类按「类型/心情/年代/地区/榜单/特色」分组，歌曲标签用可勾选芯片。
- 数据汇总卡片、操作弹窗提示，响应式粉色毛玻璃界面。

## 关于前台数据更新（暂无实时推送）

当前后台与用户前台之间**没有实时更新机制**。在后台新增、编辑、上下架或删除歌曲 / 歌手 / 专辑 / 分类后，数据会立刻写入数据库，但**已经打开的前台页面不会自动刷新**。

要让前台看到最新数据，需要回到用户前台页面**手动刷新**（按 F5 或重新进入页面），前台才会重新向后端拉取最新数据并渲染出来。

> 如需「后台一改、前台自动更新」，可后续再引入定时轮询（polling）或 SSE / WebSocket 推送，现阶段暂未实现。
...
│   │   │   └── ArtistSearchSelect.vue  # 歌手搜索联想选择器
│   │   ├── styles.css           # 后台全局样式
│   │   └── views/               # 后台页面
