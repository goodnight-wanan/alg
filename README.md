...
- 歌手地区与头像维护、分类按分组（类型/心情/年代/地区/榜单/特色）的增删改查、专辑封面上传与替换。
- 管理后台按「歌手/分类/专辑」「上传/录入歌曲」「歌曲列表」三个顶层 tab 分区，歌曲标签使用分组可勾选芯片，并提供数据汇总与弹窗提示。
...
- 歌手地区与分类类型 migration 已应用并回填现有数据。
- 歌手头像、元数据 CRUD、分类类型、专辑封面和数据汇总接口已通过真实 PostgreSQL E2E。
- 后台管理的新布局、分组标签、弹窗提示与数据汇总已通过真实浏览器冒烟验收。
